...
def initial_setup():
    psswd = '12345678'
    salt = bcrypt.gensalt()
    password = genrate_hash_password(psswd.encode('utf-8'),salt)
    ...
    if not to_execute:
        ...
        database.commit()
        query = f"insert into users(c_id,fullname,email,password,contact_no,working_under,dob,isactive,role_id,created_at,salt) values(0,'SuperAdmin','super@admin.com','{password}','1234567890',0,'2000-08-13',1,0,'{salt}','{datetime.datetime.now()}')"
